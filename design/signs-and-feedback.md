...
| Select hand card | Show selected card state, valid transparent placement previews, invalid red X placement overlays and current hold action. |
...
| Valid placement | Place the selected tile from its transparent preview, update valid-cell hints, check closure immediately. |
| Invalid placement | Show the attempted selected-tile footprint with a red X overlay and explain edge mismatch, occupied cell or unreachable placement without blocking future clicks. |
...
| Gold earned | Update gold immediately and log source: closure, strike, field resource or monster bounty. |
...
| Shop/card buy | Show gold price, card preview, affordability/bought state, buy confirmation feedback and deck/discard update. |
...
- Gold is the current post-result shop currency, not a battle survival resource.
...
- Battle win rewards pay configured monster bounties; card shop prices live in `configs/cards.json`.
...
- Add reroll/remove services only after base card buying is readable.
- Tune prices and offer weights after manual shop playtest.
