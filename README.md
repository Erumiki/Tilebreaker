...
- серые тайлы — нейтральная земля: при включенном `grayWildcardPlacement` они могут касаться серого или пустого края цветного тайла и служат fill/setup, но не закрывают цветной выход границы;
...
- `configs/game.json` — глобальные настройки tile-battle: размер поля, размер руки, стартовое HP игрока, размер стартовой колоды/recipe, opening `drawBag`, формула урона, бонусы за большую зону и серые тайлы внутри закрытия, путь к активному tile manifest, debug-сглаживание добора, gray wildcard placement, очистка доски между раундами, восстановление после dead-end, off-color leap и число битв в забеге.
