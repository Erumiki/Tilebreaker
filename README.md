...
- `configs/game.json` — глобальные настройки tile-battle: размер поля, размер руки, стартовое HP игрока, размер стартовой колоды, формула урона, путь к активному tile manifest, debug-сглаживание добора, очистка доски между раундами, восстановление после dead-end, off-color leap и число битв в забеге.
