...
- `configs/game.json` — глобальные настройки tile-battle: размер поля, размер руки, стартовое HP игрока, размер стартовой колоды/recipe, формула урона, бонусы за большую зону и серые тайлы внутри закрытия, путь к активному tile manifest, debug-сглаживание добора, gray wildcard placement, очистка доски между раундами, восстановление после dead-end, off-color leap и число битв в забеге.
