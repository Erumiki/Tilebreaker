...
- Artist-facing instructions live in `design/art-mvp-brief.md`. Art can replace PNGs in place, but must not change tile topology data (`matrix`, `edges`, `color`, `pattern`, `special` rules or scoring semantics) in manifests/configs.
...
- Latest implementation pass created the MVP art contract and placeholder pack at `assets/art_mvp/`, with technical artist guidance in `design/art-mvp-brief.md`. Actual card shop buying remains unimplemented.
